...
class GuideCollection implements OutputInterface
{
    ...
    public function fetchCollections() {
        $statement = $this->connection->prepare("SELECT c.collection_id, c.title, c.description, c.shortform FROM collection c ORDER BY c.collection_id DESC");
        $statement->execute();
    ...
    public function fetchCollectionGuides($collection_id) {
        $statement = $this->connection->prepare("SELECT s.subject_id, s.subject, s.description, s.shortform, s.type, cs.sort 
                                                  FROM subject s 
